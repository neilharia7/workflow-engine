...
filter_loading_messages_in_cli_response = get_config_boolean_value("rest_api_plugin",
                                                                   "FILTER_LOADING_MESSAGES_IN_CLI_RESPONSE", True)
airflow_rest_api_plugin_http_token_header_name = get_config_string_value("rest_api_plugin",
...
apis_metadata = [
	...
	},
	{
		...
		"http_method": ["GET", "POST"],
		"arguments": [
			{"name": "set", "description": "Set a Pool. Please enter the pool name, slot count and description",
			 "form_input_type": "custom_input",
			 "fields": [{"pool_name": "Pool Name"}, {"slot_count": "Slot Count"}, {"pool_description": "Description"}],
			 "required": False},
...
class REST_API_Response_Util():
	...
	# Gets the Base Response object with all required response fields included.
	# To be used at the beginning of the REST Call.
	@staticmethod
...
class REST_API(get_baseview()):
	...
	def is_visible(self):
		if web_authentication_enabled and current_user.is_authenticated == False:
			...
			if not web_authentication_enabled:
				return True
	...
	def execute_cli(self, base_response, api_metadata):
		...
		# appending arguments to the airflow_cmd_split array and setting arguments aside in the end_arguments array
		# to be appended onto the end of airflow_cmd_split
		...
		if dag_file and dag_file.filename.endswith(".py"):
			...
			if os.path.isfile(save_file_path) and not force:
				...
				return REST_API_Response_Util.get_400_error_response(
					base_response, "The file '" + save_file_path + "' already exists on host '" + hostname + "'.")
	...
	# This will call the direct function corresponding to the web endpoint
	# '/admin/airflow/refresh' that already exists in Airflow
	...
	# This will call the direct function corresponding to the web endpoint
	# '/admin/airflow/refresh_all' that already exists in Airflow
	def refresh_all_dags(self, base_response):
		...
		return REST_API_Response_Util.get_200_response(base_response=base_response,
		                                               output="All DAGs are now up to date")
